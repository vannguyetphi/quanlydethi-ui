  ...
  actions: {
    ...
    async addQuestionToExam({ examId, subjectId, questionIds }) {
      try {
        await Promise.all(
          questionIds.map((qId) =>
            api.post("/questionDetails", { examId, subjectId, questionId: qId })
          )
        );
        Notify.create({
          type: "positive",
    ...
    async getExamQuestions({ examId }) {
      const res = await api.get("/questionDetails/getQuestionDetails", {
        params: { examId },
      });
      this.questionDetails = res.data.data;
    },
    async getExamSubjectQuestions({ examId, subjectId }) {
      const res = await api.get("/questionDetails/getExamSubjectQuestions", {
